...
class Zzz extends CI_Controller {
    ...
    function index() {
        $data['main'] = 'pages/zzz';
        $data['title'] = "zzz";
        $data['og_title'] = "zzz $name yyy";
        $data['og_description'] = "zz ss dffff asadas";
        $data['og_img'] = asset_url().'img/sample/err.png';
        $data['og_url'] = base_url() . 'zzz';
        $scope = 'public_profile,publish_actions,user_friends';
        $login = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "zzz/app", "scope" => $scope));
        $data['login'] = $login;
...
//image merge
                ...
                $dest = Imagecreatefromjpeg(asset_url() . 'img/zzz/'.$per.'.jpg');
                ...
                imagettfstroketext($dest, 28, 0, 96, 96, $white, $black, $font_path, $fullname, 2);
...
//                Imagepng($dest);
                imagejpeg($dest, 'uploads/zzz/result_' . $user_profile['id'] . '.jpg');
                Imagedestroy($dest);
                Imagedestroy($src);
                ...
                redirect(base_url() . 'zzz/result/' . $user_id . '/' . $name);
    ...
    function result($user_id, $name) {
        ...
        $data['og_title'] = "zzz $name yyy";
        $data['og_description'] = "zz ss dffff asadas";
        $data['og_img'] = base_url() . 'uploads/zzz/result_' . $user_id . '.jpg';
        $data['og_url'] = base_url() . 'zzz/result/' . $user_id . '/' . $name . '?share=1';
        $data['user_id'] = $user_id;
        $data['name'] = $name;
        $data['login'] = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "zzz/app"));
        $this->load->view('index', $data);
        $fb = new FacebookDebugger();
        $fb->reload(base_url() . 'zzz/result/' . $user_id . '/' . $name);
...
//echo "logged in";
            ...
            if( array_key_exists('publish_actions', $arr) && $arr['publish_actions'] == 'granted' ) { //comprende? :) yes boss
                ...
                $fdate = date("Y-m-d");
                $sharelink =  base_url() . 'zzz/result/' . $user_id . '/' . $name . '?share=1';
                $this->user = $this->facebook->getUser();
