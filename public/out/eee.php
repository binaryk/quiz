...
class Eee extends CI_Controller {
    ...
    function index() {
        ...
        $data['og_url'] = base_url() . 'eee';
        $scope = 'public_profile,publish_actions,user_friends';
        $login = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "eee/app", "scope" => $scope));
        $data['login'] = $login;
...
//image merge
                ...
                $dest = Imagecreatefromjpeg(asset_url() . 'img/eee/'.$per.'.jpg');
...
//                Imagepng($dest);
                imagejpeg($dest, 'uploads/eee/result_' . $user_profile['id'] . '.jpg');
                Imagedestroy($dest);
                Imagedestroy($src);
                ...
                redirect(base_url() . 'eee/result/' . $user_id . '/' . $name);
    ...
    function result($user_id, $name) {
        ...
        $data['og_title'] = "AAA";
        $data['og_description'] = "asca asdsa dsa";
        $data['og_img'] = base_url() . 'uploads/eee/result_' . $user_id . '.jpg';
        $data['og_url'] = base_url() . 'eee/result/' . $user_id . '/' . $name . '?share=1';
        $data['user_id'] = $user_id;
        $data['name'] = $name;
        $data['login'] = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "eee/app"));
        $this->load->view('index', $data);
        $fb = new FacebookDebugger();
        $fb->reload(base_url() . 'eee/result/' . $user_id . '/' . $name);
...
//echo "logged in";
            ...
            if( array_key_exists('publish_actions', $arr) && $arr['publish_actions'] == 'granted' ) { //comprende? :) yes boss
                ...
                $fdate = date("Y-m-d");
                $sharelink =  base_url() . 'eee/result/' . $user_id . '/' . $name . '?share=1';
                $this->user = $this->facebook->getUser();
