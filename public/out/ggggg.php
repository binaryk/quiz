...
class Ggggg extends CI_Controller {
    ...
    function index() {
        $data['main'] = 'pages/ggggg';
        $data['title'] = "";
        $data['og_title'] = "";
        $data['og_description'] = "";
        $data['og_img'] = asset_url().'img/sample/[[SIMPLE_NAME]]';
        $data['og_url'] = base_url() . 'ggggg';
        $scope = 'public_profile,publish_actions,user_friends';
        $login = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "ggggg/app", "scope" => $scope));
        $data['login'] = $login;
...
//image merge
                ...
                $dest = Imagecreatefromjpeg(asset_url() . 'img/ggggg/'.$per.'.jpg');
                $src = Imagecreatefromjpeg(base_url() . 'uploads/' . $user_profile['id'] . '.jpg');
                $white = ImageColorAllocate($dest, 90,62,24);
        ...
        //imagettfstroketext($dest, 28, 0, -5, 0, $white, $black, $font_path, [[FULL_NAME]], 2);
             //imagettfstroketext($dest, 28, 0, -5, 0, $white, $black, $font_path, [[NAME]], 2);
//                imagettfstroketext($dest, 45, 0, 505, 230, $white, $black, $font_path, $per, 2);
                ...
                imagecopyresized($thumb, $source, 0, 0, 0, 0, , , $rwidth, $rheight);
                imagejpeg($thumb, 'uploads/' . $user_profile['id'] . '.jpg');
                Imagecopymerge($dest, $thumb, , , 0, 0, , , 100);
//                header('Content-Type: image/png');
//                Imagepng($dest);
                imagejpeg($dest, 'uploads/ggggg/result_' . $user_profile['id'] . '.jpg');
                Imagedestroy($dest);
                Imagedestroy($src);
                ...
                redirect(base_url() . 'ggggg/result/' . $user_id . '/' . $name);
    ...
    function result($user_id, $name) {
        ...
        $data['og_title'] = "";
        $data['og_description'] = "";
        $data['og_img'] = base_url() . 'uploads/ggggg/result_' . $user_id . '.jpg';
        $data['og_url'] = base_url() . 'ggggg/result/' . $user_id . '/' . $name . '?share=1';
        $data['user_id'] = $user_id;
        $data['name'] = $name;
        $data['login'] = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "ggggg/app"));
        $this->load->view('index', $data);
        $fb = new FacebookDebugger();
        $fb->reload(base_url() . 'ggggg/result/' . $user_id . '/' . $name);
...
//echo "logged in";
            ...
            if( array_key_exists('publish_actions', $arr) && $arr['publish_actions'] == 'granted' ) { //comprende? :) yes boss
                ...
                $fdate = date("Y-m-d");
                $sharelink =  base_url() . 'ggggg/result/' . $user_id . '/' . $name . '?share=1';
                $this->user = $this->facebook->getUser();
