...
class Catlook extends CI_Controller {
...
//image merge
                ...
                $dest = Imagecreatefromjpeg(asset_url() . 'img/catlook/'.$per.'.jpg');
...
//                Imagepng($dest);
                imagejpeg($dest, 'uploads/catlook/result_' . $user_profile['id'] . '.jpg');
                Imagedestroy($dest);
                Imagedestroy($src);
                ...
                redirect(base_url() . 'catlook/result/' . $user_id . '/' . $name);
    ...
    function result($user_id, $name) {
        ...
        $data['name'] = $name;
        $data['login'] = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "catlook/app"));
        $this->load->view('index', $data);
        $fb = new FacebookDebugger();
        $fb->reload(base_url() . 'catlook/result/' . $user_id . '/' . $name);
...
//echo "logged in";
            ...
            if( array_key_exists('publish_actions', $arr) && $arr['publish_actions'] == 'granted' ) { //comprende? :) yes boss
                ...
                $fdate = date("Y-m-d");
                $sharelink =  base_url() . 'catlook/result/' . $user_id . '/' . $name . '?share=1';
                $this->user = $this->facebook->getUser();
