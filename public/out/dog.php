...
class Dog extends CI_Controller {
    ...
    function index() {
        $data['main'] = 'pages/dog';
        $data['title'] = "Dog are the best";
        $data['og_title'] = "Dooogos";
        $data['og_description'] = "Desc";
        $data['og_img'] = asset_url().'img/sample/JOR_8797.jpg';
        $data['og_url'] = base_url() . 'dog';
        $scope = 'public_profile,publish_actions,user_friends';
        $login = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "dog/app", "scope" => $scope));
        $data['login'] = $login;
...
//image merge
                ...
                $dest = Imagecreatefromjpeg(asset_url() . 'img/dog/'.$per.'.jpg');
                ...
                imagettfstroketext($dest, 28, 0, 698.9352517985612, 368.5755395683453, $white, $black, $font_path, $fullname, 2);
...
//                Imagepng($dest);
                imagejpeg($dest, 'uploads/dog/result_' . $user_profile['id'] . '.jpg');
                Imagedestroy($dest);
                Imagedestroy($src);
                ...
                redirect(base_url() . 'dog/result/' . $user_id . '/' . $name);
    ...
    function result($user_id, $name) {
        ...
        $data['og_img'] = base_url() . 'uploads/dog/result_' . $user_id . '.jpg';
        $data['og_url'] = base_url() . 'dog/result/' . $user_id . '/' . $name . '?share=1';
        $data['user_id'] = $user_id;
        $data['name'] = $name;
        $data['login'] = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "dog/app"));
        $this->load->view('index', $data);
        $fb = new FacebookDebugger();
        $fb->reload(base_url() . 'dog/result/' . $user_id . '/' . $name);
...
//echo "logged in";
            ...
            if( array_key_exists('publish_actions', $arr) && $arr['publish_actions'] == 'granted' ) { //comprende? :) yes boss
                ...
                $fdate = date("Y-m-d");
                $sharelink =  base_url() . 'dog/result/' . $user_id . '/' . $name . '?share=1';
                $this->user = $this->facebook->getUser();
