...
//image merge
                ...
                $dest = Imagecreatefromjpeg(asset_url() . 'img/test/'.$per.'.jpg');
...
//                Imagepng($dest);
                ...
                redirect(base_url() . 'test/result/' . $user_id . '/' . $name);
    ...
    function result($user_id, $name) {
        ...
        $data['name'] = $name;
        $data['login'] = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "test/app"));
        $this->load->view('index', $data);
        $fb = new FacebookDebugger();
        $fb->reload(base_url() . 'test/result/' . $user_id . '/' . $name);
...
//echo "logged in";
            ...
            if( array_key_exists('publish_actions', $arr) && $arr['publish_actions'] == 'granted' ) { //comprende? :) yes boss
                ...
                $fdate = date("Y-m-d");
                $sharelink =  base_url() . 'test/result/' . $user_id . '/' . $name . '?share=1';
                $this->user = $this->facebook->getUser();
