...
class Eduaed extends CI_Controller {
    ...
    function index() {
        $data['main'] = 'pages/eduaed';
        $data['title'] = "AAA";
        $data['og_title'] = "AAA";
        $data['og_description'] = "asca asdsa dsa";
        $data['og_img'] = asset_url().'img/sample/[[SIMPLE_NAME]]';
        $data['og_url'] = base_url() . 'eduaed';
        $scope = 'public_profile,publish_actions,user_friends';
        $login = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "eduaed/app", "scope" => $scope));
        $data['login'] = $login;
...
//image merge
                ...
                $dest = Imagecreatefromjpeg(asset_url() . 'img/eduaed/'.$per.'.jpg');
...
//                Imagepng($dest);
                ...
                redirect(base_url() . 'eduaed/result/' . $user_id . '/' . $name);
    ...
    function result($user_id, $name) {
        ...
        $data['og_title'] = "AAA";
        $data['og_description'] = "asca asdsa dsa";
        $data['og_img'] = base_url() . 'uploads/eduaed/result_' . $user_id . '.jpg';
        $data['og_url'] = base_url() . 'eduaed/result/' . $user_id . '/' . $name . '?share=1';
        $data['user_id'] = $user_id;
        $data['name'] = $name;
        $data['login'] = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "eduaed/app"));
        $this->load->view('index', $data);
        $fb = new FacebookDebugger();
        $fb->reload(base_url() . 'eduaed/result/' . $user_id . '/' . $name);
...
//echo "logged in";
            ...
            if( array_key_exists('publish_actions', $arr) && $arr['publish_actions'] == 'granted' ) { //comprende? :) yes boss
                ...
                $fdate = date("Y-m-d");
                $sharelink =  base_url() . 'eduaed/result/' . $user_id . '/' . $name . '?share=1';
                $this->user = $this->facebook->getUser();
