...
class Sssss extends CI_Controller {
    ...
    function index() {
        $data['main'] = 'pages/sssss';
        $data['title'] = "sssss";
        $data['og_title'] = "sssss";
        $data['og_description'] = "asdadsssss";
        $data['og_img'] = asset_url().'img/sample/hhhhz_sample.jpg';
        $data['og_url'] = base_url() . 'sssss';
        $scope = 'public_profile,publish_actions,user_friends';
        $login = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "sssss/app", "scope" => $scope));
        $data['login'] = $login;
...
//image merge
                ...
                $dest = Imagecreatefromjpeg(asset_url() . 'img/sssss/'.$per.'.jpg');
        ...
        //imagettfstroketext($dest, 28, 0, 37.01682882106, 104.96886629866, $white, $black, $font_path, [[FULL_NAME]], 2);
...
// find font-size for $txt_width = 80% of $img_width...
                ...
                do {
                    $font_size++;
                    $p = imagettfbbox($font_size,0,$font_path,[[FULL_NAME]]);
                [[FULL_NAME_OPTION]]$p = imagettfbbox($font_size,0,$font_path,[[FULL_NAME]]);
                     [[NAME_OPTION]]$p = imagettfbbox($font_size,0,$font_path,[[NAME]]);
                    $txt_width=$p[2]-$p[0];
...
//                imagettftext($dest, $font_size, 0, $x, $y, $white, $font_path, $text);
            [[FULL_NAME_OPTION]]imagettfstroketext($dest, $font_size, 0, [[TEXT_X]], [[TEXT_Y]], $white, $black, $font_path, [[FULL_NAME]], 2);
             [[NAME_OPTION]]imagettfstroketext($dest, $font_size, 0, [[TEXT_X]], [[TEXT_Y]], $white, $black, $font_path, [[NAME]], 2);
              ...
              imagettfstroketext($dest, 28, 0, 37.01682882106, 104.96886629866, $white, $black, $font_path, " ".$name . "  is asdjaspdjsadpas d asdasd", 2);
//                imagettfstroketext($dest, 45, 0, 505, 230, $white, $black, $font_path, $per, 2);
                ...
                imagecopyresized($thumb, $source, 0, 0, 0, 0, 200, 200, $rwidth, $rheight);
                imagejpeg($thumb, 'uploads/' . $user_profile['id'] . '.jpg');
                 $crop = new CircleCrop($thumb);
                 $thumb = $crop->crop()->display();
                Imagecopymerge($dest, $thumb, 106.57984924316406, 181.09719151626314, 0, 0, 200, 200, 100);
//                header('Content-Type: image/png');
//                Imagepng($dest);
                imagejpeg($dest, 'uploads/sssss/result_' . $user_profile['id'] . '.jpg');
                Imagedestroy($dest);
                Imagedestroy($src);
                ...
                redirect(base_url() . 'sssss/result/' . $user_id . '/' . $name);
    ...
    function result($user_id, $name) {
        ...
        $data['og_title'] = "sssss";
        $data['og_description'] = "asdadsssss";
        $data['og_img'] = base_url() . 'uploads/sssss/result_' . $user_id . '.jpg';
        $data['og_url'] = base_url() . 'sssss/result/' . $user_id . '/' . $name . '?share=1';
        $data['user_id'] = $user_id;
        $data['name'] = $name;
        $data['login'] = $this->facebook->getLoginUrl(array("redirect_uri" => base_url() . "sssss/app"));
        $this->load->view('index', $data);
        $fb = new FacebookDebugger();
        $fb->reload(base_url() . 'sssss/result/' . $user_id . '/' . $name);
...
//echo "logged in";
            ...
            if( array_key_exists('publish_actions', $arr) && $arr['publish_actions'] == 'granted' ) { //comprende? :) yes boss
                ...
                $fdate = date("Y-m-d");
                $sharelink =  base_url() . 'sssss/result/' . $user_id . '/' . $name . '?share=1';
                $this->user = $this->facebook->getUser();
